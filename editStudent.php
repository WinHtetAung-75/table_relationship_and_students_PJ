...
<form class=" mt-5 mx-auto w-[300px]" action="updateStudent.php" method="post">
    <input type="hidden" name="student_id" value="<?= $student["id"] ?>">
    <div class="mb-6 ">
        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
        <input value="<?= $student["name"] ?>" required type="text" id="name" name="name" class="bg-gray-50 border w-full  border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
    </div>
    <div class="mb-6 ">
    ...
    <div class="mb-6 ">
        ...
        <select id="course_id" name="course_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full  p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            <option value="<?= $student["course_id"] ?>">Choose A Course</option>
            <?php
            ...
            ?>
                <option <?= $student["course_id"] == $course["id"] ? 'selected' : '' ?> value="<?= $course["id"] ?>">
                    <?= $course["title"]  ?>
                </option>
            <?php endwhile ?>
        </select>
    ...
    <div class="mb-6 ">
        ...
        <select id="batch_id" name="batch_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full  p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            ...
            ?>
                <option <?= $batch["id"] == $student["batch_id"] ? 'selected' : '' ?> value="<?= $batch["id"] ?>
             "><?= $batch["name"] ?></option>
            <?php endwhile ?>
        </select>
    </div>
    ...
    <div class=" flex justify-end mt-10">
        <button type="submit" class="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Update Student</button>
    </div>
